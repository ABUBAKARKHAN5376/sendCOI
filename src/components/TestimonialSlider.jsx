...
const testimonials = [
  {
    ...
    content: `As an agency owner managing hundreds of commercial clients, Send COI has been a game changer for our operations. What used to take hours chasing emails, manually issuing Certificates of Insurance, and handling urgent requests now happens in seconds.`,
  },
  {
    title: 'Business Owners',
    tags: ['Corporation', 'Nonprofit', 'LLC', 'Other'],
    ...
    name: 'Mark H.',
    role: 'Commercial Insurance Agent',
    content: `The platform is simple to use, incredibly responsive, and designed with both the agency and the insured in mind. Our clients appreciate the freedom to generate their own COIs from their smartphones, and our team saves countless hours every week.`,
  },
   {
    title: 'Business Owners',
    tags: ['Corporation', 'Nonprofit', 'LLC', 'Other'],
...
export default function ClientTestimonialSlider() {
  ...
  return (
    <Swiper
      ...
      slidesPerView={1.1}
      breakpoints={{
        700: { slidesPerView: 1.5, spaceBetween: 115 },
        1230: { slidesPerView: 2, spaceBetween: 135 },
        1500: { slidesPerView: 2.4, spaceBetween: 180 },
      }}
          ...
          >
            ...
            <div className="w-[60%] max-[1650px]:w-[67%] max-[850px]:w-full h-full bg-white rounded-2xl flex flex-col justify-between p-5">
              <div className="flex items-center gap-5 pb-4">
                <div
                  className="h-[100px] w-[120px] max-[1650px]:h-[80px] bg-center bg-cover rounded-xl"
                  style={{ backgroundImage: `url(${item.image})` }}
                />
                <div>
                  <b className="text-2xl font-medium max-[1650px]:text-lg">{item.name}</b>
                  <p className="font-medium text-[rgba(0,0,0,0.3)] mt-1 max-[1650px]:text-[15px] max-[1650px]:mt-0.5">
                    {item.role}
                  </p>
                </div>
              </div>
              <p className="text-lg leading-snug max-[1650px]:text-md overflow-y-auto max-[500px]:text-sm">
                {item.content}
              </p>
            </div>
